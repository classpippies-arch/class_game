# flappy_use_screenshot_bg.py
"""
Flappy — fixed background (Screenshot 2025-11-16 131426.png), home-music fixed,
in-game music selectable from repo MP3s. Safe JS injection (no f-string JS).
"""
...
st.set_page_config(page_title="Flappy (Screenshot BG + Home Music)", layout="wide")
st.title("Flappy — Fixed Screenshot Background, Home Music & Selectable In-Game Music")
st.write("Home music (menu) plays the fixed track. In-game music is chosen from repo via sidebar.")

# ---------- CONFIG: change if your filenames differ ----------
BG_NAME = "Screenshot 2025-11-16 131426.png"   # ← using the screenshot you uploaded
HOME_MUSIC_NAME = "audioblocks-mind-game-or-quiz-tension-back.mp3"  # change if actual name differs
...
# fallback: if HOME_MUSIC_NAME not present, pick first mp3
if not Path(HOME_MUSIC_NAME).exists():
    HOME_MUSIC_NAME = mp3_files[0]

# ---------- Sidebar UI ----------
st.sidebar.header("Music & Player")
game_music_selected = st.sidebar.selectbox("Choose in-game music (from repo)", mp3_files, index=0)
st.sidebar.markdown("**Home music** (menu) will be: `" + HOME_MUSIC_NAME + "` (fixed).")
player_file = st.sidebar.file_uploader("Upload player sprite (optional) — png/jpg", type=["png","jpg","jpeg"])
player_data_url = ""
...
# background URL (visual in canvas)
bg_url = ""
...
play_home_clicked = st.button("Play Home Music (if browser blocked autoplay)")

# ---------- Safe HTML + JS template with placeholders ----------
html_template = r"""
<!doctype html>
<html>
<head>
...
<title>Flappy — Screenshot BG + Music</title>
<style>
...
(function(){
  ...
  function tryPlayHomeAuto(){ if(!homeAudio.src) return; homeAudio.play().then(()=>{homeAudioAllowed=true}).catch(()=>{homeAudioAllowed=false}); }
  tryPlayHomeAuto();

  if(PLAY_HOME_FROM_PY === true){ homeAudio.play().catch(()=>{}); }
  ...
  let pipes=[]; let spawnTimer=0; let score=0; let best=0;
  ...
  function startCountdown(sec){
    ...
    function tick(){
      n--;
      if(n>=1){ countVal.innerText = n; beep(800,0.04); setTimeout(tick,1000); }
      else { overlay.style.visibility='hidden'; startGame(); }
    }
  ...
  function loop(ts){
    ...
    if(!paused){
      ...
      for(let i=pipes.length-1;i>=0;i--){
        ...
        if(!pipes[i].scored && pipes[i].x + 60 < bird.x){
          score++; pipes[i].scored = true; scoreSnd();
      ...
      if(dead){
        playing=false; try{ gameAudio.pause(); }catch(e){} hitSnd();
    ...
    if(PLAYER_URL && playerImg && playerImg.complete && playerImg.src){
      try{ ctx.save(); const rot = Math.max(-0.7, Math.min(0.7, bird.vel/12)); ctx.translate(bird.x,bird.y); ctx.rotate(rot); ctx.drawImage(playerImg, -bird.w/2, -bird.h/2, bird.w, bird.h); ctx.restore(); }
      catch(e){ ctx.fillStyle='yellow'; ctx.beginPath(); ctx.ellipse(bird.x,bird.y,18,14,0,0,Math.PI*2); ctx.fill(); }
    } else { ctx.fillStyle='yellow'; ctx.beginPath(); ctx.ellipse(bird.x,bird.y,18,14,0,0,Math.PI*2); ctx.fill(); }
  ...
  // start if python clicked
  if(START_FROM_PY === true) startCountdown(3);

  // if python asked to play home music
  if(PLAY_HOME_FROM_PY === true) homeAudio.play().catch(()=>{});
  else { setTimeout(function(){ if(!homeAudioAllowed){ const btn=document.createElement('button'); btn.innerText='Play Home Music'; btn.style.position='absolute'; btn.style.left='12px'; btn.style.bottom='12px'; btn.style.zIndex='80'; btn.style.padding='8px 12px'; btn.style.borderRadius='8px'; btn.style.background='rgba(0,0,0,0.6)'; btn.style.color='white'; btn.onclick=function(){ homeAudio.play().catch(()=>{}); this.remove(); }; document.body.appendChild(btn); } },900); }
...
paste_area = st.text_area("Paste copied JSON (or paste an integer):", height=140)
        ...
        try:
            ...
            if isinstance(obj, dict) and "scores" in obj and obj["scores"]:
                ...
                try:
                    ...
                    st.error("Failed to write on server: " + str(e))
            else:
